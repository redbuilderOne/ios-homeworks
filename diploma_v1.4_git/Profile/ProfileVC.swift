...
// MARK: - PRESENT POSTVIEW
    @objc func presentPostView() {
        present(ExpandedPostVC(), animated: true)
    }
}
...
//        print("TAP ON ", tableViewLikesLabel.text)
//        print("TAP ON ", selectedPost[0])
//        print("TAP ON ", selectedPost[3])

// ПЕРЕХОД ПО ТАПУ НА ЯЧЕЙКУ
//        tableView.deselectRow(at: indexPath, animated: true)
//        guard indexPath.row == 1 else { return }
//        let vc = ProfileVC()
//        navigationController?.pushViewController(vc, animated: true)
        //
//        guard let cell = tableView.dequeueReusableCell(withIdentifier: Cells.postCellID, for: indexPath) as? PostTableViewCell else { return }
...
//        cell?.tableViewLikesLabel.tag = indexPath.row
//        cell?.countLikes(post: firstPost)


        // TODO: - ЗДЕСЬ БУДЕТ ФУНКЦИЯ ПЕРЕХОДА НА СТРАНИЦУ ПОСТА И УВЕЛЧЕНИЕ КОЛИЧЕСТВА ПРОСМОТРОВ
//        print("\(String(describing: cell)) is tapped")
//        cell?.set(post: firstPost)
    }
...
//            cell1.tableViewLikesLabel.tag = indexPath.row

            let tapGesture = UITapGestureRecognizer(target: self, action: #selector(presentPostView))
